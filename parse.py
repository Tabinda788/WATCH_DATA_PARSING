...
jsonFilePath = "Output_Test/henrietta_site_user_test.json"
with open(jsonFilePath, 'w', encoding='utf-8') as jsonf:
        jsonf.write(json.dumps(row_order))

def replace_text(mumtiply_num):
  ...
  for elem in root.getiterator():
    ...
    try:
      if attribute['name'] == 'ThreadGroup.num_threads':
        ...
        elem.text = elem.text.replace(elem.text, str(int(elem.text)*mumtiply_num))
        #print("printing string prop attrib",elem.attrib)
       # print("printing string prop attrib",elem.text)
    except:
      pass
  tree.write('Output_Test/henrietta_site_user_test_{}u_{}r.jmx'.format(mumtiply_num*len(thread_group),mumtiply_num*len(thread_group)), encoding='latin-1')
